 ...
 * Mirrors `pickAndImportFolder` (PR #974) but targets the
 * `POST /api/projects/:id/working-dir` endpoint, which re-points an
 * existing project at a new local folder. Same security contract:
 *   - HMAC token bound to the trimmed picker output.
 *   - Single lazy retry on `503 DESKTOP_AUTH_PENDING` after a fresh
 *     `registerDesktopAuth()` call.
 *   - All other failures (4xx, 5xx, network) return a structured failure
 *     so the renderer toast surfaces the daemon's reason.
 */
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  // Atomic counterpart to dialog:pick-and-import for the "清空并替换目录"
  // (replace working directory) gesture exposed by `WorkingDirPill`.
  // Same trust boundary: picker + HMAC mint + POST are a single
  // main-process transaction so a compromised renderer can't name an
  // arbitrary baseDir and re-point an existing project at it. The
  // daemon endpoint (`POST /api/projects/:id/working-dir`) refuses
  // tokenless requests once any desktop has registered a secret;
  // before this IPC existed, the renderer-driven flow would 403 in
  // packaged builds.
  ipcMain.handle(
